...
def pre_process_topics(topics: pd.DataFrame, mode: str, **kwargs) -> pd.DataFrame:
    ...
    - cutoff: given the threshold keyword argument (default = 0), all values for topics shares below that threshold are set to this value
    - highest: given n as the number keyword argument of topics (default = 1), the n topics with highest shares are kept and the others are set to zero
    - drop: columns specified in keyword argument columns are dropped
    """
    if mode == "cutoff":
        threshold = kwargs["threshold"]
        topics = pd.concat([topics.drop(columns=['id']).clip(lower = threshold), topics['id']], axis=1)
    elif mode == "highest":
